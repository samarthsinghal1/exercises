...
subString start end str
    | null str = []
    | absStart > end = ""
    | otherwise  = drop absStart (take (end+1) str)
    where
        absStart = max 0 start
...
-}
lowerAndGreater :: (Ord a, Show a) => a -> [a] -> [Char]
lowerAndGreater n list = show n ++ " is greater than " ++ show smallElems ++ " elements and lower than " ++ show bigElems ++ " elements"
    where
        smallElems = length (filter (< n) list)
        bigElems = length (filter (> n) list)
